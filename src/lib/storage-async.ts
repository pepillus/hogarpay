...
const pagoFromDB = (row: any): Pago => ({
  ...
  horasTrabajadas: row.horas_trabajadas ? parseFloat(row.horas_trabajadas) : undefined,
...
export const getEmpleadoById = async (id: string): Promise<Empleado | undefined> => {
  if (isSupabaseEnabled() && supabase) {
    const { data, error } = await supabase
      .from('empleados')
      .select('*')
...
export const getTarifaByEmpleadoId = async (empleadoId: string): Promise<Tarifa | undefined> => {
  if (isSupabaseEnabled() && supabase) {
    const { data, error } = await supabase
      .from('tarifas')
      .select('*')
...
export const importBackup = async (data: BackupData): Promise<void> => {
  if (isSupabaseEnabled() && supabase) {
    ...
    await supabase.from('tarifas').delete().neq('id', '00000000-0000-0000-0000-000000000000');
    await supabase.from('empleados').delete().neq('id', '00000000-0000-0000-0000-000000000000');
    
    // Insertar empleados
    if (data.empleados?.length) {
      await supabase.from('empleados').insert(data.empleados.map(empleadoToDB));
    }
    
    // Insertar tarifas
    if (data.tarifas?.length) {
      await supabase.from('tarifas').insert(data.tarifas.map(tarifaToDB));
    }
    
    // Insertar pagos
    if (data.pagos?.length) {
      await supabase.from('pagos').insert(data.pagos.map(pagoToDB));
    }
    return;
  }
